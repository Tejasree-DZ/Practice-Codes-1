...
from core_service.core_apis_server.exceptions import NotFoundException, Err
...
class BaseService(Generic[ModelT]):
    ...
    @property
    def secret(self):
        return getattr(self._request.state, "cluster_secret", None)
    ...
    def get_by_id(self, record_id) -> ModelT:
        ...
        if not record:
            raise NotFoundException(
                Err.OC0014, [self.model_type.__name__, record_id])
        return record
